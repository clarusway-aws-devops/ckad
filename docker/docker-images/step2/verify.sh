...
if [[ $(docker image inspect localhost/myapp:v1 -f '{{json .RepoTags}}' | jq -r '.[]' | cut -d: -f2 | grep v1) = 'v1' \
&& $(docker image inspect localhost/myapp:latest -f '{{json .RepoTags}}' | jq -r '.[]' | cut -d: -f2 | grep latest) = 'latest' \
